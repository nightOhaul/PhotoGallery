...
//to store the data usage in DataStore
class PreferencesRepository private constructor(
    private val dataStore: DataStore<Preferences> //instance of PreferenceRepository to extract the value for key
) {
    ...
    //to check whether polling is running or not
    ...
    //creating an instance of the preference repository(singleton instance)
    companion object {
        ...
        fun get(): PreferencesRepository{
            return INSTANCE ?: throw IllegalStateException(
                "preference repository must be initialized"
            )
        }
...
// in preferencesRepository we are defining the code in which we will store the last searched word in searchbar, it is done by "shared preference" and with help of
//the datastore library. and to use this data we will use Flow and data store uses co-routines